// Bounded retries with exponential backoff; 429 honors Retry-After but still counts against maxAttempts.
export async function fetchWithRetry(fetchFn, url, { maxAttempts = 3, baseDelayMs = 100, sleep = defaultSleep } = {}) {
  ...
  for (;;) {
    ...
    if (res.status === 429) {
      if (attempt >= maxAttempts) return { ok: false, error: `http ${res.status}` };
