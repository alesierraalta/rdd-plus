...
| F12 | `bench/cases/g01-inventory-reserve/KEY.json` (before 6116c29): the case declared suite `go test ./...`, which cannot observe a data race, so its race defect can never flip a test outcome and the catch measure marks it missed however good the agent's concurrency test is | silently wrong answer (the benchmark under-reports catching on one of 27 defects) | yes | E22 | fixed | alesierraalta / 2026-09-10 | suite is now `go test -race ./...`; three consecutive scorings of the same workspace agree at 2/2 where the value used to vary | 6116c29 |
| F11 | with the plan persisted, the flow reported more than it pinned: 24 of 27 reported against 20 caught, two cases reporting a defect with no discriminating test | silently wrong answer (a finding nobody can re-run is the failure mode the whole skill exists to remove) | fixed | E21, E23 | fixed | alesierraalta / 2026-09-10 | 3.2 rule 13: every confirmed finding names a promoted test; 26 of 26 reported defects now name one and 22 are caught | 1c19591 |
| F16 | I replaced the installed Stop hook with the full CLI without changing its wiring: the old binary ran the gate with no arguments, the new one needs `gate`, so every Stop printed usage and exited 2 | visible error (the gate stopped running the moment it gained the feature it was installed for) | yes | E26 | fixed | alesierraalta / 2026-09-10 | settings.json rewired to `... testing-gate gate` after a backup; doctor now runs the wired command instead of matching its shape | pending commit |
| F15 | the plan's structure lives in a file the flow may never open: the three plans that wrote prose had not read `assets/test-plan-template.md`, while the one compliant plan read it three times, and the prose section names (`Routing Ledger`, `Execution Ratio`) come from the SKILL's Output Contract, which describes the chat reply | silently wrong answer (two competing descriptions of the structure, and the one always in context is the wrong one) | yes | E25 | fixed | alesierraalta / 2026-09-10 | `rdd-plus plan init` writes the skeleton and `plan check` names every breach, so the format stops depending on which file was read | e77bfaa |
| F14 | 3.3 raised catching to 24 of 27 and dropped reporting to 17: three plans abandoned the template table for prose sections and one cited only the symbol and its test, so the location the scorer needs was never written | silently wrong answer (a plan nothing can parse is not a persisted plan; it reads as "found nothing") | yes | E24 | open | — | candidate for 3.4: the template's tables are the format, prose replaces no row, and the finding cell opens with `path:line` | pending |
| F13 | rule 13 did not say which direction the pinning test must run: n03 satisfied it with three characterization tests that assert the DEFECTIVE behaviour, so a correct fix turns them red and the defect is pinned in place instead of pinned down | silently wrong answer (the test resists the fix; it is deleted when the bug is fixed) | fixed | E23, E24 | fixed | alesierraalta / 2026-09-10 | 3.3 rule 13 states the assertion points at the contract and the runs happen red then green; rule 4 draws the boundary. No inverted test appeared in the 3.3 run and catching rose from 22 to 24 of 27 | 6f40f9d |
