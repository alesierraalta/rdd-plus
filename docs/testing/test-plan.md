...
| F13 | rule 13 does not say which direction the pinning test must run: n03 satisfied it with three characterization tests that assert the DEFECTIVE behaviour, so a correct fix turns them red and the defect is pinned in place instead of pinned down | silently wrong answer (the test resists the fix; it is deleted when the bug is fixed) | yes | E23 | open | — | candidate for 3.3: a pinning test asserts the CORRECT behaviour, so it is red now and green after the fix; a characterization test is rule 4's separate, labelled instrument | pending |
