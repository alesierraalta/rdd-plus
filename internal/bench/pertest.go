...
func perTestCommand(suite string) (argv []string, parser func(string) map[string]bool) {
	...
	case fields[0] == "node" && len(fields) > 1 && fields[1] == "--test":
		return append(fields[:2:2], append([]string{"--test-reporter=tap"}, fields[2:]...)...), parseTAP
	case fields[0] == "go" && len(fields) > 1 && fields[1] == "test":
		return append(fields[:2:2], append([]string{"-json"}, fields[2:]...)...), parseGoJSON
	}
	return fields, nil
}
...
// format yields the single entry "suite".
func perTest(dir, suite string, timeout time.Duration) (map[string]bool, int, string) {
	...
	if parser == nil {
		return map[string]bool{"suite": code == 0}, code, out
	}
	tests := parser(out)
	if len(tests) == 0 {
