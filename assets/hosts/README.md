...
`rdd-plus gate` serves both. Wire it with `rdd-plus sync` on Claude Code, or by merging
...
`opencode/rdd-plus.ts` subscribes to `session.idle`, runs `rdd-plus check`, and on a non-zero
exit shows a toast and appends the report to the prompt.

A host with neither still gets everything except "what did THIS session do": `rdd-plus check`
...
|---|---|---|
| Claude Code | Stop command hook | yes: wired, run, exit code checked by `rdd-plus doctor` |
| Anything with a shell | `rdd-plus check` | yes |
